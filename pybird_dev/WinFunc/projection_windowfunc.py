...
class Projection(object):
    ...
    def __init__(
        ...
        #window_fourier_name = r"Beutleretal_window_kspace_z3_NGC.dat",
        path_to_window=None,
        #path_to_window = r"mnt\c\Users\s4393083\pybird\tbird",
        window_configspace_file=None, #Original
        #window_configspace_file = "Beutleretal_window_z3_NGC.dat",
        binning=False,
        fibcol=False,
        Nwedges=0,
        zz=None,
        ...
        #Added to force the code to calculate
        self.co.with_cf = False
        print("PRINT CO")
        print(self.co.with_cf)
        self.xout = xout
    ...
    def setWindow(self, load=True, save=True, Nl=3, withmask=True, windowk=0.05):
        ...
        if self.co.with_cf:
            try:
                ...
                swindow_config_space=np.loadtxt("Ql_z3_NGC_rebinned_500bins.dat", skiprows=1, usecols=range(0,2,3,4)) #Manually defined like a scrub
                print(swindow_config_space)
                print("Success! Found Mask file!")
            except:
                #print("Error: can't load mask file: %s." % self.window_configspace_file)
                print("Error: can't load mask file: %s." % swindow_config_space)
                compute = False

        else:
            print("PATH TO WINDOW:")
            #print(self.path_to_window)
            ...
            #window_fourier_file = os.path.join(self.path_to_window, "%s_Nl%s_kmax%.2f.npy") % (
                ...
                #self.co.Nl,
                #self.co.kmax,
            window_fourier_file = os.path.join("%s_Nl%s_kmax%.2f.npy") % ( #Defined w.r.t local path, reads window func
                #r"Ql_z3_NGC_rebinned_500bins",
                self.window_fourier_name,
                self.co.Nl,
                self.co.kmax,
            )
            ...
            if not load:  # do not change to else
                ...
                print("trying to access Correlator: ")
                print(self.window_configspace_file)
                if self.window_configspace_file is None:
                    print("Error: please specify a configuration-space mask file.")
                    compute = False
                else:
                    try:
                        print("made it to config space definition")
                        print("swindow config space:")
                        print(self.window_configspace_file)
                        #swindow_config_space=np.loadtxt("Beutleretal_window_z1_NGC.dat", skiprows=1, usecols=range(0,4)) #currently only reads in first three data columns
                        #swindow_config_space=np.loadtxt("Window_BOSS_CMASS_NGC_z057.dat", skiprows=0) #Skipping dummy in case not supposed to be used
                        swindow_config_space=np.loadtxt(self.window_configspace_file)
                        print("loaded swindow_config_space:")
                        print(swindow_config_space)
                        #print(np.loadtxt(self.window_configspace_file))
                        #swindow_config_space = np.loadtxt(self.window_configspace_file)
                    except:
                        print("Error: can't load mask file: %s." % self.window_configspace_file)
                        compute = False
        ...
        if compute is True:
            ...
            #print(Qp)
            #print("saving Qp")
            #np.savetxt("Qp.dat", Qp)
            Qal = np.einsum("alp,ps->als", Calp[:Nl, :Nl, :Nl], Qp)
            #print("Printing Qal")
            #print(Qal)
            #np.save("FirstRoundQalOutput", Qal)
            #testing output:
            #self.co.with_cf=True
            if self.co.with_cf:
                print("if self.co.with_cf accepted!")
                self.Qal = interp1d(sw, Qal, axis=-1, kind="cubic", bounds_error=False, fill_value="extrapolate")(
                    self.co.s)
                print("Printing self.Qal")
                print(self.Qal)

            else:
                print("made it to else, calculating and saving mask")
                ...
                print("Printing self.co.Nk")
                print(self.co.Nk)
                print("Printing self.co.k")
                print(self.co.k)
                print("printing self.P")
                print(self.p)
                print("printing self.pPow")
                print(self.pPow)
                print("printing self.co.s")
                print(self.co.s)
                print(sw)
                print("length of sw = %lf" %len(sw))
                ...
                if save:
                    print(self.Wal)
                    #print("Saving mask: %s" % window_fourier_file) #problems with finding the directory
                    #np.save(window_fourier_file, self.Wal)
                    print("saving mask:")
                    #np.save("Rebinned_RR_Window_z3_kspace_NGC_Nl3_kmax0.60.npy", self.Wal)
                    #np.save("Ql_z3_NGC_rebinned_500binsNGC_Nl3_kmax0.60.npy", self.Wal)
                    np.save(window_fourier_file, self.Wal)
    ...
    def Window(self, bird):
        ...
        """
        print("REACHED WINDOW APPLICATION TO BIRD SPECTRA")
        if self.with_window:
            if self.co.with_cf:
                if bird.with_bias:
