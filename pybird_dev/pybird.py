...
class Correlator(object):
    def __init__(self, config_dict=None, load_engines=True):
        ...
        self.config_catalog = {
            ...
            "with_nlo_bias": Option("with_nlo_bias", bool, description="With next-to-leading bias.", default=False),
            "with_assembly_bias": Option("with_assembly_bias", bool, description="With assembly bias.", default=False),
    ...
    def set(self, config_dict, load_engines=True):
        ...
        if load_engines:
            ...
            self.__load_engines(only_common=True)
    ...
    def compute(self, cosmo_dict, module=None):
        ...
        if self.config["skycut"] == 1:
            self.bird = Bird(
                ...
                with_nlo_bias=self.config["with_nlo_bias"],
                with_assembly_bias=self.config["with_assembly_bias"],
                co=self.co,
            )
            ...
            if "w" in self.config["output"]:
                ...
                if self.config["with_redshift_bin"]:
                    self.projection.redshift(
                        self.bird, self.cosmo["Dz"], self.cosmo["fz"], self.cosmo["DAz"], self.cosmo["Hz"]
                    )
                elif self.config["with_AP"]:
        ...
        if self.config["skycut"] == 1:
            ...
            if as_dict:
                ...
                if "Pk" in self.config["output"]:
                    ...
                    if self.config["with_stoch"]:
                        correlator_cache["st"] = self.bird.Pstl
                    if self.config["with_nlo_bias"]:
                        correlator_cache["nlo"] = self.bird.Pnlol
                    ...
                    if self.config["with_nlo_bias"]:
                        correlator_cache["nlo"] = self.bird.Cnlol
                    ...
                    if self.config["with_nlo_bias"]:
                        correlator_cache["nlo"] = self.bird.wnlo
            ...
            if as_dict:
                ...
                if "Pk" in self.config["output"]:
                    ...
                    if self.config["with_stoch"]:
                        correlator_cache["st"] = [self.birds[i].Pstl for i in range(self.config["skycut"])]
                    if self.config["with_nlo_bias"]:
                        correlator_cache["nlo"] = [self.birds[i].Pnlol for i in range(self.config["skycut"])]
                elif "Cf" in self.config["output"]:
                    ...
                    if self.config["with_nlo_bias"]:
                        correlator_cache["nlo"] = [self.birds[i].Cnlol for i in range(self.config["skycut"])]
                elif "w" in self.config["output"]:
                    ...
                    if self.config["with_nlo_bias"]:
                        correlator_cache["nlo"] = [self.birds[i].wnlo for i in range(self.config["skycut"])]
                return correlator_cache
    ...
    def get(self, bias=None):
        ...
        if self.config["skycut"] == 1:
            ...
            if "Pk" in self.config["output"]:
                ...
                return [self.birds[i].w for i in range(self.config["skycut"])]
    ...
    def getmarg(self, b1, model=1):
        def marg(loop, ct, b1, f1, Pst=None, model=model):
            ...
            if self.config["with_time"]:
                Pb3 = loop[3] + b1 * loop[7]
            else:
                Pb3 = f * loop[8] + b1 * loop[16]
            ...
            if "w" not in self.config["output"]:
                if self.config["multipole"] >= 2:
                    m = np.vstack([m, 2 * (f * ct[1 + 3] + b1 * ct[1]).reshape(-1) / self.config["km"] ** 2])
                if self.config["multipole"] >= 3:
                    m = np.vstack([m, 2 * (f * ct[2 + 3] + b1 * ct[2]).reshape(-1) / self.config["km"] ** 2])
                if self.config["with_stoch"]:
                    # if "Cf" in self.config["output"]:
                    #     if model == 5: m = np.vstack([m, Pst[0].reshape(-1)])
                    # elif "Pk" in self.config["output"]:
                    if model <= 4:
                        m = np.vstack([m, Pst[2].reshape(-1)])
                    if model == 1:
                        m = np.vstack([m, Pst[0].reshape(-1)])
                    if model == 3:
                        m = np.vstack([m, Pst[1].reshape(-1)])
                    if model == 4:
                        m = np.vstack([m, Pst[1].reshape(-1), Pst[0].reshape(-1)])

            return m

        if self.config["skycut"] == 1:
            if "Pk" in self.config["output"]:
                return marg(self.bird.Ploopl, self.bird.Pctl, b1=b1, f1=self.bird.f, Pst=self.bird.Pstl)
            elif "Cf" in self.config["output"]:
                return marg(self.bird.Cloopl, self.bird.Cctl, b1=b1, f1=self.bird.f, Pst=self.bird.Cstl)
            elif "w" in self.config["output"]:
                return marg(self.bird.wloop, self.bird.wct, b1=b1, f1=self.bird.f)
        elif self.config["skycut"] > 1:
            if "Pk" in self.config["output"]:
                return [
                    marg(self.birds[i].Ploopl, self.birds[i].Pctl, b1=b1[i], f1=self.birds[i].f, Pst=self.birds[i].Pstl)
                    for i in range(self.config["skycut"])
                ]
            elif "Cf" in self.config["output"]:
                return [
                    marg(self.birds[i].Cloopl, self.birds[i].Cctl, b1=b1[i], f1=self.birds[i].f, Pst=self.birds[i].Cstl)
                    for i in range(self.config["skycut"])
                ]
            elif "w" in self.config["output"]:
                return [
                    marg(self.birds[i].wloop, self.birds[i].wct, b1=b1[i], f1=self.birds[i].f)
                    for i in range(self.config["skycut"])
                ]
                ...
                if self.config["skycut"] == 1:
                    ...
                    for i in range(self.config["skycut"]):
                        ...
                        if len(self.config["xdata"]) == 1:
                            xdata = self.config["xdata"][i]
                        elif len(self.config["xdata"]) is self.config["skycut"]:
                            xdata = self.config["xdata"][i]
                        else:
                            xdata = self.config["xdata"]
    ...
    def __is_cosmo_conflict(self):
        ...
        if self.config["with_AP"] and not self.config["with_redshift_bin"]:
            ...
            if self.config["skycut"] == 1:
                ...
                len(self.cosmo["DA"]) is not self.config["skycut"] or len(self.cosmo["H"]) is not self.config["skycut"]
            ):
        ...
        if self.config["with_redshift_bin"]:
            ...
            if self.config["skycut"] == 1:
                if len(self.cosmo["Dz"]) is not len(self.config["zz"]) or len(self.cosmo["fz"]) is not len(
                    self.config["zz"]
                ):
                    raise Exception("Please specify 'Dz' and 'fz' with same length as 'zz'. ")
            elif (
                len(self.cosmo["Dz"]) is not self.config["skycut"] or len(self.cosmo["fz"]) is not self.config["skycut"]
            ):
            ...
            if self.config["with_AP"]:
                if self.cosmo["DAz"] is None or self.cosmo["Hz"] is None:
                    raise Exception(
                        "You asked to account the galaxy counts distribution and apply the AP effect. Please specify 'DAz' and 'Hz'. "
                    )

                if self.config["skycut"] == 1:
                    if len(self.cosmo["DAz"]) is not len(self.config["zz"]) or len(self.cosmo["Hz"]) is not len(
                        self.config["zz"]
                    ):
                        raise Exception("Please specify 'DAz' and 'Hz' with same length as 'zz'. ")
                elif (
                    len(self.cosmo["DAz"]) is not self.config["skycut"]
                    or len(self.cosmo["Hz"]) is not self.config["skycut"]
                ):
    ...
    def __is_bias_conflict(self, bias=None):
        ...
        if "m" in self.config["output"]:
            ...
            if self.config["with_nlo_bias"]:
                try:
                    self.bias["bnlo"] = self.cosmo["bias"]["bnlo"]
                except:
                    raise Exception("Please specify the next-to-leading bias 'bnlo'.  ")

            if self.config["with_assembly_bias"]:
                try:
                    self.bias["bq"] = self.cosmo["bias"]["bq"]
                except:
                    raise Exception("Please specify the assembly bias 'bq'.  ")
    ...
    def __is_config_conflict(self):
        ...
        if self.config["wedge"] is not 0:
            self.config["multipole"] = 3  # enforced
        ...
        if self.config["with_quintessence"]:
            self.config["with_exact_time"] = True
