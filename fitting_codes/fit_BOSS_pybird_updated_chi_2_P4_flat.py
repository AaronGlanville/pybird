#Code to run flat fits off of our curved grid. 
#We do this in practice by simply fixing the omk parameter to 0 
#in our sampler. Need to make sure this doesn't affect our evidence
#calculation

#First attempt- avoid giving omega_k a sport in our 
#dynesty params, and just fix it in log-likelihood/grid codes
...
def prior_transform(u, birdmodels):
    ...
    for i in range(nz):
        x[4 + (2*i)] = u[4 + (2*i)] * 3.0 #0-3, flat
        x[5 + (2*i)] = (u[5 + (2*i)] - 0.5) * 8.0 #[-4, 4], flat
...
if __name__ == "__main__":
    ...
    start = [[2.6, birdmodels[0].valueref[1], birdmodels[0].valueref[2], birdmodels[0].valueref[3]]]
    if birdmodels[0].pardict["do_marg"]:
        for i in range(len(pardict["z_pk"])):
    ...
    #code for nelder-mead optimisation:
    ndim = 4 + (2*len(pardict["z_pk"])) #flat, only using b1, c2

    dsampler = NestedSampler(lnlike, prior_transform, ndim, logl_args=[birdmodels, fittingdata, plt], ptform_args=[birdmodels], nlive=100, bound='multi', sample='unif')
    dsampler.run_nested()
    dres=dsampler.results
    print(dres)
    np.save("parallel_static_output_with_resum_4th_order_flat/100x100_local_pybird_z1_z_z3_updated_chi_2_order_4_kmax_0p20_0p20_0p10_knl_0p8_flat_true_data_"+str(input_num), dres)
