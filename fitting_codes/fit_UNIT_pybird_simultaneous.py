#FIRST ATTEMPT AT SIMULTANEOUS FIT USING NGC AND SGC LOW-Z BOSS Pk
...
def do_emcee(func, start, birdmodel, fittingdata):
    ...
    chainfile = "Test_fit_UNIT_Pybird"
    print(chainfile)
    ...
    sampler = emcee.EnsembleSampler(nwalkers, nparams, func, args=[birdmodel, fittingdata], backend=backend)
...
def lnpost(params, birdmodel, fittingdata):
    ...
    like = lnlike(params, birdmodel, fittingdata, fittingdataSGC)
    return prior + like
...
#def lnlike(params, birdmodel, fittingdata, fittingdataSGC, plt):
def lnlike(params, birdmodel, fittingdata, fittingdataSGC):
    if birdmodel.pardict["do_marg"]:
        ...
        bs = [
            ...
            params[-6],
            params[-5],
            params[-4] * fittingdata.data["shot_noise"], #Double check how second shot_noise fits in here
    ...
    Pi = birdmodel.get_Pi_for_marg(Ploop, bs[0], fittingdata.data["shot_noise"], fittingdata.data["x_data"]) #check how to add shot noise for both samples

    
    NGC_chi_squared = birdmodel.compute_chi2(P_model_interp, Pi, fittingdata.data)
    chi_squared = birdmodel.compute_chi2(P_model_interp, Pi, fittingdata.data) + birdmodel.compute_chi2(P_model_interp, Pi, fittingdataSGC.data)
    ...
    print("NGC only chi_squared")
    print(NGC_chi_squared)
    print("NGC + SGC chi_squared")
    print(chi_squared)
...
if __name__ == "__main__":
    ...
    configfileSGC = sys.argv[2]
    plot_flag = int(sys.argv[3])
    pardict = ConfigObj(configfile)
    pardictSGC = ConfigObj(configfileSGC)
    print(pardictSGC)

    # Just converts strings in pardicts to numbers in int/float etc.
    pardict = format_pardict(pardict)
    pardictSGC = format_pardict(pardictSGC)

    # Set up the data
    fittingdata = FittingData(pardict, shot_noise=float(pardict["shot_noise"]))
    fittingdataSGC = FittingData(pardictSGC, shot_noise=float(pardict["shot_noise"]))
