...
def lnprior(params, birdmodels): 
    ...
    #print("priors before Omega_k")
    #print(priors)

    #Because omega_k_0 in the wrong order
    priors += np.where(np.logical_or(omega_k > lower_bounds[4], omega_k < upper_bounds[4]), -np.inf, 0.0)

    #print("priors after Omega_k")
    #print(priors)

    # BBN (D/H) inspired prior on omega_b
    #omega_b_prior = -0.5 * (omega_b - 0.02205) ** 2 / 0.00049 ** 2
    omega_b_prior = -0.5 * (omega_b - 0.02235) ** 2 / 0.00049 ** 2
    # omega_b_prior = 0.0
...
def lnlike(params, birdmodels, fittingdata, plt):
    ...
    for i in range(nz):
        ...
        print("(np.array([ln10As, h, omega_cdm, omega_b, omega_k])")
        print(np.array([ln10As, h, omega_cdm, omega_b, omega_k]))
    ...
    if np.random.rand() < 1:
        print(params[:, 0], bs, chi_squared_print, len(fittingdata.data["fit_data"]))
...
if __name__ == "__main__":
    ...
    #configfile = "../config/order_4_grid_configs/tbird_NGCSGC_curved_z1_mock_mean.txt"
    #configfile = "../config/order_4_grid_configs/tbird_NGCSGC_curved_z1_true_data.txt"
    configfile = "../config/order_4_grid_configs/tbird_NGCSGC_curved.txt"
    plot_flag = 0
