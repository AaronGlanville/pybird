...
sys.path.append("../")
from fitting_codes.fitting_utils_P4 import (
    FittingData,
    BirdModel,
    create_plot,
    update_plot,
...
def prior_transform(u, birdmodels):
    x = np.array(u)
    priors = np.zeros(np.shape(x[1]))

    #using center of grid
    ...
    #mu, sigma = 0.02205, 0.00049
    #x[3] = scipy.stats.norm.ppf(u[3], loc=mu, scale=sigma)

    #I initially set the width of the grids to be too small- so I'll need to truncate the normal to the grid size (rip)

    mu, sigma = 0.02205, 0.00049
    low, high = lower_bounds[3], upper_bounds[3]
    low_n, high_n = (low-mu)/sigma, (high-mu)/sigma #standardize
    x[3] = scipy.stats.truncnorm.ppf(u[3], low_n, high_n, loc=mu, scale=sigma)

    x[4] = (u[4] * (upper_bounds[4] - lower_bounds[4])) + lower_bounds[4]

    #Now we place priors on the remaining parameters:
    nz = len(birdmodels[0].pardict["z_pk"])
    for i in range(nz):
        x[5 + (2*i)] = u[5 + (2*i)] * 3.0 #0-3, flat
        x[6 + (2*i)] = (u[6 + (2*i)] - 0.5) * 8.0 #[-4, 4], flat
...
def lnprior(params, birdmodels):
    ...
    omega_b_prior = -0.5 * (omega_b - 0.02205) ** 2 / 0.00049 ** 2
    # omega_b_prior = 0.0
...
def lnlike(params, birdmodels, fittingdata, plt):
    ...
    #################################
    # For individual fits, non-vectorised
    #################################
...
if __name__ == "__main__":
    ...
    #We have to dynamically read in the datafile indexed by input_num
    bianchi_suffix = ["_COMPnbar_TIC_250_460_260_120_kmax_0p3.dat", "_COMPnbar_TIC_190_360_210_120.dat",
                      "_COMPnbar_TIC_290_540_300_120.dat", "_COMPnbar_TIC_160_430_240_120.dat",
                      "_COMPnbar_TIC_340_650_360_120.dat", "_COMPnbar_TIC_190_500_280_120.dat"]

    #bianchi_suffix = ["_COMPnbar_TIC_250_460_260_120_kmax_0p3.dat", "_COMPnbar_TIC_190_360_210_120.dat"]
    
    datafiles = np.loadtxt(pardict["datafile"], ndmin=1, dtype=str)
    print("datafiles before reassignment")
    print(datafiles)
    #we only want to apply this to the first len(z_pk) terms
    new_datafiles = []
    for i in range(len(pardict["z_pk"])):
        print(str(datafiles[i]))
        print(str(array_num))
        print(str(bianchi_suffix[i]))
        print(str(datafiles[i]) + str(array_num) + str(bianchi_suffix[i]))
        new_element = str(datafiles[i]) + str(array_num) + str(bianchi_suffix[i])
        new_datafiles.append(new_element)
    print("datafiles after reassignment")
    print(new_datafiles)
    pardict["datafile"] = new_datafiles
    pardict = format_pardict(pardict)
    print(pardict)
    ...
    output_name = "nelder_mead_fits/nelder_mead_z1_z2_z3_output"
    result = do_optimization(lambda *args: -lnpost(*args), start, output_name, array_num)
    '''

    # Differential evolution
    lower_bounds = birdmodels[0].valueref - birdmodels[0].pardict["order"] * birdmodels[0].delta
    upper_bounds = birdmodels[0].valueref + birdmodels[0].pardict["order"] * birdmodels[0].delta
    #original start- 1 redshift bin/2 chunks
    '''start = (
    ...
    output_name = "basin_hopping_fits/basin_hopping_z1_z2_z3_individual_output_P4_kmax_0p15_knl_0p8_corrected_redshifts_"
    result, bestfit = do_optimization(lambda *args: -lnpost(*args), start, output_name, array_num)
    chi_2_best_fit = lnpost(bestfit)
    final_result = np.append(bestfit, chi_2_best_fit)
    np.savetxt(str(output_name)+str(array_num)+".txt", final_result[np.newaxis], fmt='%lf', delimiter=' ')
