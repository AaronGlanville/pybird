...
class BirdModel:
    def __init__(self, pardict, redindex=0, template=False, direct=False, window=None):
        ...
        self.k_m, self.k_nl = 0.6, 0.6
        #self.k_m, self.k_nl = 0.8, 0.8
        #self.k_m, self.k_nl = 1.0, 1.0
        #self.eft_priors = np.array([20.0, 20.0, 40.0, 40.0, 20.0, 20.0, 20.0])
        
        self.eft_priors = np.array([4.0, 4.0, 8.0, 8.0, 0.2, 4.0, 4.0]) #Doubled widths with shrunk ce terms
        
        #self.eft_priors = np.array([2.0, 2.0, 4.0, 4.0, 2.0, 2.0, 2.0]) #This seemed to worsen our posterior, but give closer optimisation. reverting to original widths
        ...
        if self.pardict["do_marg"]:
            ...
            if self.pardict["do_corr"]:
                ...
                Pi = np.array(
                    [
                        Pb3,  # *b3
                        2*Pcct / self.k_nl ** 2,  # *cct
                        2*Pcr1 / self.k_m ** 2,  # *cr1
                        2*Pcr2 / self.k_m ** 2,  # *cr2
...
class FittingData:
    ...
    def read_pk(self, inputfile, step_size, skiprows):
        ...
        print("final Pk data for fitting")
    ...
    def read_data(self, pardict):
        ...
        #Now we rescale, as per Percival 2014
        n_b = len(fit_data) #number of bins
        n_s = 2048 #number of mocks used to compute cov_matrix
        D = ((n_b+1)/(n_s - 1))
        cov_inv = (1-D) * cov_inv
        
        print("n_b, n_s, cov_inv factor of Percival 2014")
        print(n_b, n_s, D)
...
def do_optimization(func, start, output_name, array_num):
    ...
    result = differential_evolution(func, start, init='random', strategy='best1bin', popsize=15, maxiter=3000, tol=0.001, recombination=0.7, mutation=0.5)

    print("#-------------- Best-fit----------------")
    print(result)

    #also: save best fit to file (to run an array of jobs)
    bestfit = result.x
    #np.savetxt(str(output_name)+str(array_num)+".txt", bestfit[np.newaxis], fmt='%lf', delimiter=' ')
