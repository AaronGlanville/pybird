...
sys.path.append("../")
from fitting_codes.fitting_utils_NGCSGC_singleconfig import (
    FittingData_NGC_SGC,
    BirdModel,
...
def lnlike(params, birdmodel, fittingdata, plt):
    ...
    if plt is not None:
        ...
        update_plot_combined(pardict_NGC_SGC, fittingdata.data["x_data"], P_model_interp_NGC, P_model_interp_SGC, colours, "NGC+SGC_z1_combined", plt)
...
if __name__ == "__main__":
    ...
    NGC_SGC_configfile = "../config/tbird_NGC_z1_s10fixed_singlefit_singleconfig.txt"
    plot_flag = int(sys.argv[1])
    pardict_NGC_SGC = ConfigObj(NGC_SGC_configfile)
    print(pardict_NGC_SGC)

    # Just converts strings in pardicts to numbers in int/float etc.
    pardict_NGC_SGC = format_pardict(pardict_NGC_SGC)
    NGC_shot_noise = float(pardict_NGC_SGC["shot_noise_NGC"])
    SGC_shot_noise = float(pardict_NGC_SGC["shot_noise_SGC"])
    print("NGC/SGC shot noise = %lf %lf"%(NGC_shot_noise, SGC_shot_noise))

    # Set up the data
    fittingdata_combined = FittingData_NGC_SGC(pardict_NGC_SGC, NGC_shot_noise, SGC_shot_noise)
    ...
    birdmodel_combined = BirdModel(pardict_NGC_SGC, template=False)

    # Plotting (for checking/debugging, should turn off for production runs)
    plt = None
    if plot_flag:
        colours = ["b", "g", "r", "c", "n"]
        plt = create_plot_combined(pardict_NGC_SGC, colours, fittingdata_combined) #NOTE: Pardict NGC must contain both gridnames for the NGC and SGC sections
